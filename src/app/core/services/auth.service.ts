...
export class AuthService {
  ...
  private readonly medicoProfileSignal = signal<MedicoProfileWrapperDto | null>(
    null,
  );
  ...
  readonly isDoctor = computed(
    () =>
      this.user()?.roles.includes('doctor') ||
      this.user()?.roles.includes('medico'),
  );
  readonly isPatient = computed(
    () =>
      this.user()?.roles.includes('patient') ||
      this.user()?.roles.includes('paciente'),
  );
  ...
  async signup(data: SignupDto): Promise<MensajeResponseDto> {
    ...
    try {
      return await firstValueFrom(
        this.http.post<MensajeResponseDto>(this.API_URL, data),
      );
    } finally {
      this.loadingSignal.set(false);
    }
  ...
  async forgotPassword(data: ForgotPasswordDto): Promise<MensajeResponseDto> {
    ...
    try {
      return await firstValueFrom(
        this.http.post<MensajeResponseDto>(
          `${this.API_URL}/olvide-password`,
          data,
        ),
      );
    } finally {
  ...
  async resetPassword(
    ...
    try {
      return await firstValueFrom(
        this.http.post<MensajeResponseDto>(
          `${this.API_URL}/recuperar-password/${token}`,
          data,
        ),
      );
    } finally {
  ...
  async loadUserProfile(): Promise<void> {
    ...
    try {
      ...
      const enfermedadesMap =
        backendResponse.data.perfiles.paciente?.enfermedades || {};
      const enfermedades: EnfermedadPaciente[] = Object.entries(
        enfermedadesMap,
      ...
      const profile: PerfilResponseDto = {
        ...
        cedula:
          backendResponse.data.perfiles.paciente?.cedula || medicoData?.cedula,
        ...
        telefono:
          backendResponse.data.perfiles.paciente?.telefono ||
          medicoData?.telefono,
        pais: backendResponse.data.perfiles.paciente?.pais,
        genero:
          backendResponse.data.perfiles.paciente?.genero || medicoData?.genero,
        ...
        imagen:
          backendResponse.data.perfiles.paciente?.imagen ||
          medicoData?.fotoPerfil,
        enfermedades,
      };
